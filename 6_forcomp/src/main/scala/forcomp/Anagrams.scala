...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val charToGroup = w.toLowerCase groupBy (identity)
    val charToFreq = for ((char, group) <- charToGroup) yield (char, group.length)
    charToFreq.toList.sortBy { case (char, _) => char }
  }
  ...
  def combinations(occ: Occurrences): List[Occurrences] =
    occ match {
      case Nil => List(Nil)
      case (letter, freq) :: rest => {
        val partial = combinations(rest)
        partial ::: {
          for {
            n <- 1 to freq
            comb <- partial
  ...
  /** Subtracts occurrence list `y` from occurrence list `x`.
    ...
    */
  def subtract(occ: Occurrences, subset: Occurrences): Occurrences = {
    (occ, subset) match {
      case (z, Nil) => z
      case (Nil, _) => throw new IllegalArgumentException("occ must be a superset of subset")

      case ((charA, freqA) :: restA, (charB, freqB) :: restB) => {
        if (charA != charB)
          (charA, freqA) :: subtract(restA, subset)
        else {
          val diff = freqA - freqB
          if (diff > 0)
            (charA, diff) :: subtract(restA, restB)
          else if (diff == 0)
            subtract(restA, restB)
          else
            throw new IllegalArgumentException(s"negative difference for  '$charA'")
        }
      }
    }
  }
  ...
  /** Returns a list of all anagram sentences of the given sentence.
    ...
    * all the words in the sentence, and producing all possible combinations of words with those
    ...
    * anagrams.
    * For example, sentences `List("You", "olive")` and `List("olive", "you")` are different
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def anagrams(occ: Occurrences): List[Sentence] =
      if (occ.isEmpty) List(Nil)
        ...
        sent <- anagrams(subtract(occ, comb))
      } yield word :: sent
